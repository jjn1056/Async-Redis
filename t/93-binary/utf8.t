...
SKIP: {
    ...
    subtest 'UTF-8 values' => sub {
        ...
        await_f($r->set('utf8:value', $utf8));

        my $result = await_f($r->get('utf8:value'));
    ...
    subtest 'UTF-8 in hash fields' => sub {
        ...
        await_f($r->hset('utf8:hash', $field, $value));

        my $result = await_f($r->hget('utf8:hash', $field));
    ...
    subtest 'UTF-8 in list elements' => sub {
        ...
        for my $item (@items) {
            await_f($r->rpush('utf8:list', $item));
        }

        my $result = await_f($r->lrange('utf8:list', 0, -1));
    ...
    subtest 'emoji values' => sub {
        ...
        my $emoji = "🚀🎉💻🔥";
        await_f($r->set('utf8:emoji', $emoji));

        my $result = await_f($r->get('utf8:emoji'));
